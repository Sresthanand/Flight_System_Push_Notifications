...
insert into admin values('sresth','123456');
-- Clients 
...
('JAMES', 'ROBERT', 'SMITH', '+11234567890', 'JAMESROBERTSMITH@EXAMPLE.COM', '133'),
('MARY', 'ANN', 'JOHNSON', '+12345678901', 'MARYANNJOHNSON@EXAMPLE.COM', '134'),
('PATRICK', 'JAMES', 'BROWN', '+13456789012', 'PATRICKJAMESBROWN@EXAMPLE.COM', '135'),
('LINDA', 'MAY', 'DAVIS', '+14567890123', 'LINDAMAYDAVIS@EXAMPLE.COM', '136'),
('JOHN', 'WILLIAM', 'MARTIN', '+15678901234', 'JOHNWILLIAMMARTIN@EXAMPLE.COM', '137');
...
CREATE TABLE Ticket (
    ...
    departure_time TIMESTAMP, -- better to use TIMESTAMP for date and time
    gate_no INT,
    airport_code CHAR(3) -- format (A-Z)(A-Z)(A-Z)
);
--------------donezo--------

---------------F----------------------------------

-- insert into flight_status_system.ticket (seat_no,departure_time) values(62,NULL);
-- select * from ticket;
...
insert into ticket (seat_no,departure_time) values(62,NULL);
select * from ticket;
alter table ticket auto_increment=10;
delete from ticket where ticket_id=67; 
select t.nm,s.departure_time,a.gate_no,a.airport_code from schedule s, tempseatgen t,airport a where s.schedule_id=62 and a.airport_name='Atlanta Airport' order by rand() limit 1;
...
-- triggers
DROP TRIGGER IF EXISTS `flight_status_system`.`Ticket_AFTER_INSERT`;
DELIMITER $$
USE `flight_status_system`$$
CREATE DEFINER = CURRENT_USER TRIGGER `flight_status_system`.`Ticket_AFTER_INSERT` AFTER INSERT ON `Ticket` FOR EACH ROW
BEGIN
	update flight_status_system.ticket set seat_no= (select nm from flight_status_system.tempseatgen order by rand() limit 1) where seat_no is null;
END$$
DELIMITER ;
